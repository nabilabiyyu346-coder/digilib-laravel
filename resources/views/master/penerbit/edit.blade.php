...
@section('title', 'Edit Penerbit - DigiLib')
...
<div class="row">
    <div class="col-md-6">
        <div class="card">
            <div class="card-body">
                <form action="{{ route('penerbit.update', $penerbit->kodePenerbit) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('alamat') is-invalid @enderror"
                                  ...
                                  rows="3" required>{{ old('alamat', $penerbit->alamat) }}</textarea>
                        @error('alamat')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('telp') is-invalid @enderror"
                               ...
                               value="{{ old('telp', $penerbit->telp) }}" required>
                        @error('telp')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="email" class="form-control @error('email') is-invalid @enderror"
                               ...
                               value="{{ old('email', $penerbit->email) }}" required>
                        @error('email')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
